...
import FormLot from "@/Components/Main/Form/FormLot.vue";
...
const form = useForm({
    brand_id: props.lot.brand_id,
    model_id: props.lot.model_id,
    vin: props.lot.vin,
    year_release: props.lot.year_release,
    color_id: props.lot.color_id,
    mileage: props.lot.mileage,
    fuel_id: props.lot.fuel_id,
    wheel_id: props.lot.wheel_id,
    drive_id: props.lot.drive_id,
    body_type_id: props.lot.body_type_id,
    transmission_id: props.lot.transmission_id,
    engine_capacity: props.lot.engine_capacity,
    horsepower: props.lot.horsepower,
    price: props.lot.price,
    description: props.lot.description,
    is_published: props.lot.is_published,
});
...
function getPhotos() {
    axios.get(`/admin/lot/tmp-restore/${props.lot.id}`)
        .then(response => {
...
function reorderFiles(files, origin, target) {
    ...
    axios.post(`/admin/lot/tmp-reorder/${props.lot.id}`, filePositionId)
        .then((response) => {
        })
        .catch((error) => {
            console.log(error);
...
const confirmingLotDeletion = ref(false);

const confirmLotDeletion = () => {
    confirmingLotDeletion.value = true;
};

const deleteLot = (id) => {
    router.delete(route('admin-lot.destroy', id), {
        preserveScroll: true,
...
const closeModal = () => {
    confirmingLotDeletion.value = false;
    form.clearErrors();
    form.reset();
};

let toggle = ref();
toggle.value = props.lot.is_published;

function updatePublished() {
    form.is_published = toggle.value
    router.patch(route('admin-lot.updatePublished', props.lot.id), {
...
<template>
    ...
    <MainLayout>
        ...
        <div class="p-2 lg:p-4 max-w-2xl mx-auto shadow sm:rounded-2xl bg-white">
            ...
            <div class="flex justify-between pb-2">
                ...
                <div>
                    ...
                    <DangerButton @click="confirmLotDeletion">
                        Удалить
                    </DangerButton>
                </div>
            </div>
            ...
            <form :disabled="form.processing"
                  @submit.prevent="form.patch(route('admin-lot.update', lot.id))">
                    ...
                    <div class="h-full mb-8 text-gray-900">
                        ...
                        <file-pond
                            ...
                            :server="{
                                ...
                                timeout: 120000,
                                process:{
                                    url: `/admin/lot/tmp-upload/${props.lot.id}`,
                                    method: 'POST',
                                ...
                                revert:{
                                    url: `/admin/lot/tmp-revert/${props.lot.id}`,
                                    method: 'DELETE',
        ...
        <Modal :show="confirmingLotDeletion" @close="closeModal">
            <div class="p-6">
                <h2
