...
let lotCount = ref(props.lots.meta.total);
let viewFullFilter = ref(false);

function updateCatalogView() {
    axios.patch(route('lot-filter.updateCatalogView'), {catalog_view: toggle.value})
        .then(res => {
                router.reload({only: ['lots']})
...
watch((formFilter), (formFilter) => {
    axios.post(route('lot-filter.filterCount'), {formFilter: formFilter, formSearch: props.formSearch},)
        .then(response => {
            lotCount.value = response.data
        })
        .catch(error => {
            console.log(error)
...
const filterIndex = () => {
    if (route().current('admin-lots.filter') || route().current('admin-lots.index')) {
...
const cleanForm = () => {
    for (const key in formFilter) {
        if (formFilter[key] !== '') {
            return router.get(route('admin-lots.filter'));
        }
    }
}
</script>
    ...
    <div class="p-4 sm:px-5 mb-4 max-w-screen-2xl mx-auto shadow sm:rounded-2xl bg-white">
        <form @submit.prevent="filterIndex">
            ...
            <div class="flex w-full sm:hidden mt-4">
                ...
                <ButtonCyan :disabled="lotCount === 0" :type="'submit'" class="w-1/2 justify-center">
                    Показать {{ lotCount }} авто
                </ButtonCyan>
            </div>
            <div class="flex justify-between mt-4">
                ...
                <div class="hidden sm:flex gap-4">
                    ...
                    <ButtonCyan :disabled="lotCount === 0" type="submit">Показать {{ lotCount }} авто</ButtonCyan>
                </div>
            </div>
        </form>
    </div>
